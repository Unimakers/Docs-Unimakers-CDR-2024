...
La pince vise à manipuler les éléments de jeu de la Coupe de France de Robotique 2024. Ces éléments de jeu se composent de petits pots ronds métalliques (d'environ 6*6.5) et de fleurs en plastique légèrement plus petites. De petits panneaux solaires sont également à manipuler, ceux-ci doivent uniquement être tourné dans un sens mais le mécanisme dédié sera détaillé dans une autre partie.
...
L'objectif de la pince est de pouvoir manipuler les éléments de jeu. Une fois en prise, les mâchoires peuvent se lever avec l'élément de jeu et s'orienter vers l'intérieur du robot pour ydéposer les plantes. L'élément de jeu peut alors être stockée dans un barillet, que ce soit une plante ou un pot. En résumé, la pince représente la liaison entre les éléments de jeu et le robot.

# Spécification technique 

## Contraintes et exigences spécifiques à la Coupe de France de Robotique 2024

Les contraintes spécifiques à la Coupe de France de Robotique sont principalement la taille du robot et de ses systèmes embarqués, ainsi qu'une execution des tâches rapides du au peu de temps que dure l'épreuve. Pour mieux répondre à la contrainte de taille, le système doit être le plus petit possible ou exploiter au mieux les espaces disponibles dans le châssis. En revanche, un robot rapide nécessite plus de puissance de la part des moteurs.

Parmi les contraintes de la CDR, il y a également la nécessité d'avoir un robot entièrement autonome. Cela implique de pouvoir connaître en temps réel la position des moteurs et donc de la pince, ainsi que de pouvoir automatiser ces mouvements via une carte programmée. Le robot doit posséder sa propre source d'alimentation électrique. Etant donné que les objets à manipuler restent très légers et que les pièces mécaniques sont faites en PLA (matériaux assez léger), il n'y a pas d'exigence particulière en termes de puissances des moteurs; de simples moteurs à charbon seront suffisants. L'utilisation de moteur à charbon permet de limiter les problèmes liés à des besoins énergétique élevés sur le mécanisme de pince.

## Exigences de performance et de précision

La performance sera principalement jugée par la vitesse du système.

Sur le terrain de jeu, il y a 36 pots et 36 plantes répartis sur des zones prédéfinies. Ainsi, pour transporter l'intégralité des pots avec les plantes à l'intérieur, il faut effectuer 72 fois le mouvement pour les stocker et 72 fois pour les vider. En 90 secondes, cela laisse moins d'une seconde pour chaque mouvement de montée/descente du système. Il est clair que le système ne pourra pas répondre à 100 % à cette exigence, notamment si l'on tient compte du temps de déplacement jusqu'aux zones prévues. Cependant, l'objectif serait de s'en rapprocher le plus que possible.

En ce qui concerne la précision, seule la rotation autour de l'axe Y doit être précise. La précision sur l'axe Z (la hauteur) importera peu pour le système. De même, les mâchoires n'ont pas besoin d'une forte précision. Leurs matériaux souples et leur forme permettent d'absorber tout défaut de précision.
...
Le système d'ascenseur à été choisi entre deux système différents quant à son mécanisme. Le premier consistait à utiliser une tige filtée que l'on aurait fait tourné à l'aide d'un moteur brushless. Les positions hautes et basses auraient été déterminées grâce à deux capteurs poussoirs. Ce système présentait tout de même des inconvénients plus important que celui choisie actuellement. Parmis ceux-ci on peut retrouver l'alimentation des moteurs brushless et l'impossibilité de mettre en place plusieurs positions hautes à moins d'utiliser un encodeurs qui complexifie grandement la programmation. 

Le système permettant aux machoires de monter et descendre s'est donc orienté vers un servomoteur et un bras de levier sur un rail linéaire. En tournant, le moteur tire vers le haut le reste du système. La seule évolution de ce système a été de doubler le bras de levier pour le solidifier et d'ajouter de multiples perçages à des fins de test quant à la force de celui-ci en fonction de la taille du bras. Ce système présente l'avantage d'être simple à piloter, assez rapide, compact et de ne prendre de la place qu'en hauteur (à l'endroit où se situe le servomoteur), l'espace le moins utilisé sur le robot.
